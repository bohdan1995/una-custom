<ul class="bx-menu-ver bx-menu-object-<?php echo $a['object'];?> bx-clearfix">
    <?php if(is_array($a['bx_repeat:menu_items'])) for($i=0; $i<count($a['bx_repeat:menu_items']); $i++){ ?>
        <li class="bx-menu-item <?php echo $a['bx_repeat:menu_items'][$i]['name'];?> bx-def-round-corners bx-def-color-bg-hl-hover <?php echo $a['bx_repeat:menu_items'][$i]['class_add'];?>">
            <a class="<?php echo $a['bx_repeat:menu_items'][$i]['name'];?>" href="<?php echo $a['bx_repeat:menu_items'][$i]['link'];?>" onclick="<?php echo $a['bx_repeat:menu_items'][$i]['onclick'];?>"<?php echo $a['bx_repeat:menu_items'][$i]['attrs'];?>>
                <div class="bx-def-padding-sec">
                    <?php if($a['bx_repeat:menu_items'][$i]['bx_if:icon']['condition']){ ?><i class="sys-icon <?php echo $a['bx_repeat:menu_items'][$i]['bx_if:icon']['content']['icon'];?>"></i><?php } ?><?php if($a['bx_repeat:menu_items'][$i]['bx_if:image']['condition']){ ?><img style="background-image:url(<?php echo $a['bx_repeat:menu_items'][$i]['bx_if:image']['content']['icon_url'];?>)" src="http://localhost/una-test/template/images/icons/spacer.gif" /><?php } ?><span><?php echo $a['bx_repeat:menu_items'][$i]['title'];?></span>
                </div>
            </a>
        </li>
